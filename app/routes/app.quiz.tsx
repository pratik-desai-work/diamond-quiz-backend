...
export async function action({ request }: any) {
    ...
    if (intent === 'upload-option-image') {
        const file = formData.get('file') as File;
        if (!file) {
            return { error: 'No file received' };
        }

        const { mediaId, previewUrl } = await uploadImageToShopify(admin, file);
    ...
    if (intent === 'add-question') {
        await addQuestionToQuiz(Number(formData.get('quizId')), {
            ...
            description: String(formData.get('description') || ''),
            min: formData.get('min') ? Number(formData.get('min')) : null,
        });
    }
            ...
            <s-stack gap="base">
                {questions.map((q: any, index: number) => (
                    ...
                    >
                        ...
                        <s-box
                            border="base"
                            background={q.active ? 'subdued' : 'transparent'}
                            borderRadius="base"
                            padding="base"
                        >
                            <s-stack gap="base">
